...
public class InputEventInspector : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        _event.hasRepeatDelay = EditorGUILayout.Toggle("Repeats If Held", _event.hasRepeatDelay);
        ...
        _event.sendOnActive = GUILayout.Toggle(_event.sendOnActive, _activeContent, EditorStyles.miniButtonLeft);
        _event.sendOnInactive = GUILayout.Toggle(_event.sendOnInactive, _inactiveContent, EditorStyles.miniButtonMid);
        _event.sendOnTriggered = GUILayout.Toggle(_event.sendOnTriggered, _triggeredContent, EditorStyles.miniButtonMid);
        _event.sendOnReleased = GUILayout.Toggle(_event.sendOnReleased, _releasedContent, EditorStyles.miniButtonRight);
        GUILayout.EndHorizontal();
        #endregion
    ...
    private void DrawInputMethods<InputMethodType>(InputEvent _event, List<InputMethodType> _inputMethods) where InputMethodType : InputMethod, new()
    {
        ...
        if(_inputMethods != null)
        {
            ...
            if (GUILayout.Button("Add Input Method", EditorStyles.miniButton, GUILayout.Width(130), GUILayout.Height(19)))
            {
                ...
                _newInputMethod.sendOnChannels = new List<bool>(_event.channelCount);
